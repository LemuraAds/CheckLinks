...
def main(argv: List[str]) -> int:
    ...
    for campaign_id, name in client.iter_active_campaign_ids():
        print(f"Кампания: {name} (ID {campaign_id})")
        for ad in client.iter_ads(campaign_id):
            ad_id = ad.get("Id")
            for url in extract_urls_from_ad(ad):
                status, error = check_url(url, timeout=args.timeout)
                if status == 500:
                    print(f"  Объявление {ad_id}: ссылка {url} отвечает 500 (OK)")
                elif status is None:
                    any_issue = True
                    print(f"  Объявление {ad_id}: ошибка запроса {url}: {error}")
                else:
                    any_issue = True
                    print(f"  Объявление {ad_id}: ссылка {url} отвечает {status}")

    if any_issue:
        print("Найдены ссылки с отличным от 500 ответом.")
    else:
        print("Все ссылки возвращают 500.")
    return 0
